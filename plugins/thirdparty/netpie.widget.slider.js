...
(function() {
    ...
    freeboard.loadWidgetPlugin({
	    ...
	    {
		"name"          : "onhandle",
		"display_name"  : "Start on handle",
		"type"          : "boolean",
		"default_value" : 0,
		"description"   : "Start action on handle (or not), page reload needed after changing this option!",
	    },
            {
                "name"        : "color",
    ...
    var sliderWidgetPlugin = function(settings) {
        ...
        var sliderElement = $("<input id=\""+self.widgetID+"\" type=\"range\" onhandle=\""+settings.onhandle+"\" min=\""+settings.min+"\" max=\""+settings.max+"\" step=\""+settings.step+"\" value=\""+(settings.initialvalue || 0)+"\" />");
        self.autoValue = 0;
